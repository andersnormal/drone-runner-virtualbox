...
import (
	...
	"github.com/andersnormal/drone-runner-virtualbox/engine/replacer"
...
func (e *execer) Exec(ctx context.Context, spec *engine.Spec, state *pipeline.State) error {
	defer e.engine.Destroy(context.Background(), spec)

	if err := e.engine.Setup(context.Background(), spec); err != nil {
		state.FailAll(err)
		return e.reporter.ReportStage(context.Background(), state)
	}
	...
	state.FinishAll()
	if err := e.reporter.ReportStage(context.Background(), state); err != nil {
		multierror.Append(result, err)
	}
	return result
}
...
func (e *execer) exec(ctx context.Context, state *pipeline.State, spec *engine.Spec, step *engine.Step) error {
	...
	case step.RunPolicy == engine.RunOnFailure && state.Failed() == false:
		...
		return e.reporter.ReportStep(context.Background(), state, step.Name)
	...
	err := e.reporter.ReportStep(context.Background(), state, step.Name)
	if err != nil {
		return err
	}
	...
	wc := e.streamer.Stream(context.Background(), state, step.Name)
